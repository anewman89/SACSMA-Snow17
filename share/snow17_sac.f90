...
!these are in the namelist bloc &INIT_CONTROL
  ...
  character(len = 1024)		:: init_param_name	!name of initial mddel parameters for hrus
...
!variables for sorted observed streamflow
  real(dp), dimension(36500)	:: sort_streamflow
  integer(I4B),dimension(36500)	:: order
...
!for hru version
  integer(I4B)		     :: num_hru
  real(dp),dimension(100)    :: hru_area,hru_elev

!hru forcing variables
  real(dp), dimension(100,36500)		:: hru_tmin,hru_tmax,hru_vpd,hru_dayl,hru_swdown,hru_precip
!derived forcing variables
  real(dp), dimension(100,36500)		:: hru_tair,hru_pet

  real(dp), dimension(100,36500)		:: fuse_raim

  !initial parameter array for hrus
  real(sp), dimension(100,100)			:: init_params

  !bounds
  real(sp), dimension(30)   :: blp     !lower bounds on parameter set
  real(sp), dimension(30)   :: bup     !upper bounds on parameter set
...
!                            val_period,pet_coef,gage_id,opt_name
  ...
  namelist / INIT_CONTROL / dt,sim_length,init_smois,forcing_name,opt, &
                            ...
                            val_period,pet_coef,gage_id,opt_name,init_param_name
  save
end module
